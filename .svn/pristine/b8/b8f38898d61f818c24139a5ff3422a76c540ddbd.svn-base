...
class CustomerReceivableController extends Controller {
    ...
    public function actionSummary() {
        ...
        $accounts = Account::model()->findAll(array(
            'condition' => 't.account_category_id = 3 AND t.is_inactive = 0 AND t.branch_id = :branch_id',
...
//
//        $customerReceivableSummary = new CustomerReceivableSummary($account->search(), $journalAccounting->search());
//
//        $customerReceivableSummary->setupLoading($startDate, $endDate, $startAccount, $endAccount);
//        $customerReceivableSummary->setupPaging($pageSize, $currentPage);
////		$customerReceivableSummary->setupSorting();
//        $customerReceivableSummary->setupFilter($startDate, $endDate, $branchId, $startAccount, $endAccount);
////		$customerReceivableSummary->getSaldo($startDate);
//        //for list data
//        $accounts = Account::model()->findAll(array(
//            'condition' => 'account_category_id = 3 AND branch_id = :branchId',
//            'params' => array(
//                ':branchId' => $branchId
//            ),
//            'order' => 'name ASC'
...
//
//        if (isset($_GET['SaveExcel']))
//            $this->saveToExcel($customerReceivableSummary, $customerReceivableSummary->dataProvider, $startDate, $endDate);
//
//        $this->render('summary', array(
//            'account' => $account,
...
//    }
    ...
    protected function saveToExcel($accounts, $branchId, $endDate) {
        ...
        $documentProperties->setTitle('Laporan Piutang Penjualan');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Piutang Penjualan');
        ...
        $worksheet->setCellValue('A2', 'Laporan Piutang Penjualan');
        ...
        $worksheet->setCellValue('C5', 'Total Piutang');
        ...
        foreach ($accounts as $account) {
            $totalReceivable = $account->getCustomerReceivableBalance($branchId, $endDate);
            if ($totalReceivable > 100):
                ...
                $worksheet->setCellValue("C{$counter}", CHtml::encode(Yii::app()->numberFormatter->format('#,##0', $totalReceivable)));
                $counter++;
        ...
        header('Content-Disposition: attachment;filename="Laporan Piutang Penjualan.xlsx"');

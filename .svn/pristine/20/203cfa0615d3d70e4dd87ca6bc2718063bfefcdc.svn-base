...
//            ':start_date' => $startDate,
        ...
        $sql = "SELECT balance_debit - balance_credit AS total_balance
                FROM (
                    SELECT account_id, SUM(debit) AS balance_debit, 0 AS balance_credit
                    FROM " . JournalAccounting::model()->tableName() . "
                    WHERE account_id = :account_id AND date <= :end_date " . $branchConditionSql .
                    " GROUP BY account_id
                    UNION
                    SELECT account_id, 0 AS balance_debit, SUM(credit) AS balance_credit
                    FROM " . JournalAccounting::model()->tableName() . "
                    WHERE account_id = :account_id AND date <= :end_date " . $branchConditionSql .
                    " GROUP BY account_id
                ) transaction";

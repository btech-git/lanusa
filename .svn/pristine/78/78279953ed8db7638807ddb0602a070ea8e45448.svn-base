...
class SqlGenerator extends CComponent {
    ...
    public static function bankBook() {
        $sql = "SELECT dc.date, a.name AS account, dc.debit, dc.credit, dc.detail_account_id, dc.note, dc.module
				FROM
				(
					" . SqlViewGenerator::balance() . "
				) dc
				INNER JOIN " . Account::model()->tableName() . " a ON a.id = dc.detail_account_id
				WHERE dc.date BETWEEN :start_date AND :end_date AND dc.branch_id = :branch_id AND dc.is_inactive = 0 AND a.is_inactive = 0
    ...
    public static function beginningBalance() {
        $sql = "SELECT COALESCE(SUM(dc.debit) - SUM(dc.credit), 0) AS beginning_balance FROM
				...
				INNER JOIN " . Account::model()->tableName() . " a ON a.id = dc.detail_account_id
				WHERE dc.account_id = :account_id AND dc.date < :start_date AND dc.branch_id = :branch_id AND dc.is_inactive = 0 AND a.is_inactive = 0";
    ...
    public static function endingBalance() {
        $sql = "SELECT COALESCE(SUM(dc.debit) - SUM(dc.credit), 0) AS ending_balance FROM
				...
				INNER JOIN " . Account::model()->tableName() . " a ON a.id = dc.detail_account_id
				WHERE dc.account_id = :account_id AND dc.date <= :end_date AND dc.branch_id = :branch_id AND dc.is_inactive = 0 AND a.is_inactive = 0";
    ...
    public static function profitLoss() {
        $sql = "SELECT sale.amount AS sale_amount, purchase.amount AS purchase_amount, beginning_stock.amount AS beginning_stock_amount, ending_stock.amount AS ending_stock_amount, 
				...
				(
					...
					WHERE ac.account_category_type_id = 3 AND aj.date <= :end_date AND aj.branch_id = :branch_id
				) sale
				CROSS JOIN
				(
					...
					INNER JOIN " . Account::model()->tableName() . " a ON a.id = aj.account_id 
					INNER JOIN " . AccountCategory::model()->tableName() . " ac ON ac.id = a.account_category_id 
					WHERE ac.account_category_type_id = 4 AND aj.date <= :end_date AND aj.branch_id = :branch_id
				) purchase
				CROSS JOIN
				(
					SELECT COALESCE(SUM(credit) - SUM(debit), 0) as amount 
					...
					INNER JOIN " . Account::model()->tableName() . " a ON a.id = aj.account_id 
					INNER JOIN " . AccountCategory::model()->tableName() . " ac ON ac.id = a.account_category_id 
					WHERE ac.account_category_type_id = 5 AND aj.date <= :end_date AND aj.branch_id = :branch_id
				) expense
				CROSS JOIN
				(
					SELECT COALESCE(SUM(debit) - SUM(credit), 0) as amount 
					...
					INNER JOIN " . Account::model()->tableName() . " a ON a.id = aj.account_id 
					INNER JOIN " . AccountCategory::model()->tableName() . " ac ON ac.id = a.account_category_id 
					WHERE ac.account_category_type_id = 6 AND aj.date <= :end_date AND aj.branch_id = :branch_id
				) other_income
				CROSS JOIN
				(
					SELECT COALESCE(SUM(credit) - SUM(debit), 0) as amount 
					...
					INNER JOIN " . Account::model()->tableName() . " a ON a.id = aj.account_id 
					INNER JOIN " . AccountCategory::model()->tableName() . " ac ON ac.id = a.account_category_id 
					WHERE ac.account_category_type_id = 7 AND aj.date <= :end_date AND aj.branch_id = :branch_id
				) other_expense
				CROSS JOIN
				(
					...
					(
						...
						WHERE h.date < :end_date AND h.branch_id = :branch_id AND d.is_inactive = 0
					) purchase
					CROSS JOIN
					(

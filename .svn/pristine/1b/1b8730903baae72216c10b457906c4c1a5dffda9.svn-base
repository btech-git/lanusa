...
class StockLocalController extends Controller {
    ...
    public function actionSummary() {
        ...
        $stockLocalSummary = new StockLocalSummary($product->search());
        $stockLocalSummary->setupLoading();
        $stockLocalSummary->setupPaging($pageSize, $currentPage);
        $stockLocalSummary->setupSorting();
        $stockLocalSummary->setupFilter($category, $warehouseId);

        if (isset($_GET['SaveExcel'])) {
            $this->saveToExcel($stockLocalSummary, $stockLocalSummary->dataProvider, $warehouseId);
        }


        $this->render('summary', array(
            'product' => $product,
            'stockLocalSummary' => $stockLocalSummary,
    ...
    protected function reportTotalStock($dataProvider) {
        ...
        foreach ($dataProvider->data as $data)
            $grandTotal += $data->getLocalStock($warehouseId);
    ...
    protected function reportTotalStockPrice($dataProvider) {
        ...
        foreach ($dataProvider->data as $data)
            $grandTotal += $data->getLocalStockPrice($warehouseId);

        return $grandTotal;
    }

    protected function saveToExcel($stockLocalSummary, $dataProvider, $warehouseId) {
        ...
        $worksheet->setCellValue('A2', 'Laporan Stok Barang');
        ...
        foreach ($dataProvider->data as $header) {
            ...
            $worksheet->setCellValue("D{$counter}", CHtml::encode($header->getLocalStock($warehouseId)));
            $worksheet->setCellValue("E{$counter}", CHtml::encode($header->getLocalStockItemPrice($warehouseId)));
            $worksheet->setCellValue("F{$counter}", CHtml::encode($header->getLocalStockPrice($warehouseId)));
            $counter++;
        }
        ...
        $worksheet->setCellValue("D{$counter}", CHtml::encode($this->reportTotalStock($stockLocalSummary->dataProvider)));
        $worksheet->setCellValue("F{$counter}", CHtml::encode($this->reportTotalStockPrice($stockLocalSummary->dataProvider)));

        header('Content-Type: application/xlsx');
        header('Content-Disposition: attachment;filename="Laporan Stok Barang.xlsx"');

...
class Expense extends CComponent {
    ...
    public function generateCodeNumber($branchId, $currentMonth, $currentYear) {
        ...
        if ($expenseHeader !== null)
            $this->header->setCodeNumber($expenseHeader->cn_ordinal, $expenseHeader->cn_month, $expenseHeader->cn_year, $expenseHeader->branch_id);
        ...
        foreach ($this->details as $detail) {
            ...
            if ($detail->is_inactive == 0) {
                $accountingJournalDebit = AccountingJournalHelper::make(
                                'debit',
                                $this->header->getCodeNumber($expenseHeaderConstant),
                                $this->header->date,
                                $detail->account_id,

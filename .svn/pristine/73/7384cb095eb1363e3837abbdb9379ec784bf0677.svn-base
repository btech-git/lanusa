	...
	<?php
		...
		$pageSizeDropDown = CHtml::dropDownList(
			...
			array(
				...
				'onchange' => "$.fn.yiiGridView.update('purchaseReceipt-grid',{data:{pageSize:$(this).val()}});",
			)
		);
		?>
...
<?php $this->widget('zii.widgets.grid.CGridView', array(
	...
	'columns'=>array(
		array(
			...
			'filter' => '<div style="display: inline-block">' . CHtml::activeTextField($purchaseReceipt, 'cn_ordinal', array('maxLength' => 4, 'size' => 2)) . '</div>' .
						'<div style="display: inline-block"> &nbsp; /' . PurchaseReceiptHeader::CN_CONSTANT . '/ &nbsp; </div>' .
						'<div style="display: inline-block">' . CHtml::activeDropDownList($purchaseReceipt, 'cn_month', array(1 => 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII'), array('empty' => '')) . '</div>' .
						'<div style="display: inline-block"> &nbsp; / &nbsp; </div>' .
						'<div style="display: inline-block">' . CHtml::activeTextField($purchaseReceipt, 'cn_year', array('maxLength' => 2, 'size' => 2)) . '</div>',

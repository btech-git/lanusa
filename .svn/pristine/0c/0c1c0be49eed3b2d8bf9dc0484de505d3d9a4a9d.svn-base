...
<div style="font-weight: bold; text-align: center">
	...
	<div style="font-size: larger">Laporan Pajak Keluaran</div>
...
<table class="report">
	...
	<?php foreach ($saleRecapSummary->dataProvider->data as $header): ?>
		<tr class="items1">
			<td class="width1-1" style="text-align: center"><?php echo CHtml::encode($header->getCodeNumber(SaleInvoice::CN_CONSTANT)); ?></td>
			...
			<td class="width1-4" style="text-align: left"><?php echo CHtml::encode(CHtml::value($header, isset($header->deliveryHeader->saleHeader->customer->company) ? 'deliveryHeader.saleHeader.customer.company' : 'deliveryHeader.saleHeader.customer.name')); ?></td>
	...
	<tr>
		...
		<td class="width1-5" style="border-top: 1px solid; font-weight: bold; text-align: right">
            <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $this->reportGrandTotalSale($saleRecapSummary->dataProvider))); ?>
        </td>
        <td class="width1-6" style="border-top: 1px solid; font-weight: bold; text-align: right">
            <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $this->reportGrandTotalTax($saleRecapSummary->dataProvider))); ?>
        </td>
        <td class="width1-7" style="border-top: 1px solid; font-weight: bold; text-align: right">
            <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $this->reportGrandTotal($saleRecapSummary->dataProvider))); ?>
        </td>
	</tr>
</table>

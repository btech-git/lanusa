...
class MonthlyTransactionActiveRecord extends ActiveRecord {
    ...
    public function getCodeNumber($cnConstant = '') {
        ...
        $branchCode = CHtml::value($this, 'branch.code');
        return sprintf('%04d/%s/%s/%s/%02d', $this->cn_ordinal, $cnConstant, $branchCode, $arr[$month], $this->cn_year);
    }

    public function setCodeNumber($cnOrdinal, $cnMonth, $cnYear, $branchId) {
    ...
    public function setCodeNumberByNext($currentMonth, $currentYear) {
        ...
        $this->setCodeNumber($ordinal + 1, $currentMonth, $currentYear, $this->branch_id);
    }

    public function normalizeCnMonthBy($romanNum) {
        $this->cn_month = self::normalizeMonthBy($romanNum);
    }

    public static function normalizeMonthBy($romanNum) {
        ...
        if ($romanNum === '')
            ...
            $month = isset($arr[$romanNum]) ? $arr[$romanNum] + 1 : 0;
        }

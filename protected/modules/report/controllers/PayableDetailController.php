...
class PayableDetailController extends Controller {
    ...
    public function actionSummary() {
        ...
        $payableDetailSummary = new PayableDetailSummary($purchaseReceipt->search());
        ...
        $payableDetailSummary->setupFilter($filters);

        if (isset($_GET['SaveExcel']))
            $this->saveToExcel($payableDetailSummary, $branch, $payableDetailSummary->dataProvider, array('startDate' => $startDate, 'endDate' => $endDate));
    ...
    protected function saveToExcel($payableDetailSummary, $branch, $dataProvider, array $options = array()) {
        ...
        $documentProperties->setTitle('Laporan Hutang Detail');

        $worksheet = $objPHPExcel->setActiveSheetIndex(0);
        $worksheet->setTitle('Hutang Detail');
        ...
        $worksheet->setCellValue('A2', 'Laporan Hutang Detail');
        ...
        foreach ($dataProvider->data as $header) {
            ...
            foreach ($header->purchaseReceiptDetails as $detail) {
                ...
                $worksheet->setCellValue("A{$counter}", CHtml::encode($detail->receiveHeader ? $detail->receiveHeader->purchaseHeader->getCodeNumber(PurchaseHeader::CN_CONSTANT) : ''));
                ...
                $worksheet->setCellValue("F{$counter}", CHtml::encode(CHtml::value($detail, 'receiveHeader.reference')));
            ...
            $grandTotalInvoice += $header->totalReceive;
        ...
        header('Content-Disposition: attachment;filename="Laporan Hutang Detail.xlsx"');

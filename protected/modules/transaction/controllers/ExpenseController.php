...
class ExpenseController extends Controller {
    ...
    public function actionCreate() {
        ...
        $branchId = isset($_GET['ExpenseHeader']['branch_id']) ? $_GET['ExpenseHeader']['branch_id'] : '';
        ...
        if (isset($_POST['Submit']) && IdempotentManager::check()) {
            ...
            $expense->generateCodeNumber($expense->header->branch_id, Yii::app()->dateFormatter->format('M', strtotime($expense->header->date)), Yii::app()->dateFormatter->format('yy', strtotime($expense->header->date)));
    ...
    public function actionView($id) {
        ...
        if (isset($_POST['Submit']) && (int) $expense->is_approved !== 1) {
            $expense->is_approved = 1;
            $expense->date_approval = date('Y-m-d');
            $expense->admin_id_approval = Yii::app()->user->id;

            if ($expense->save()) {
                Yii::app()->user->setFlash('confirm', 'Your Quotation has been confirmed!!!');
            } else {
                Yii::app()->user->setFlash('confirm', 'Your Quotation failed to confirmed!!!');
            }
    ...
    public function actionDelete($id) {
        if (Yii::app()->request->isPostRequest) {
            ...
            JournalAccounting::model()->deleteAllByAttributes(array(
                'transaction_number' => $expense->getCodeNumber($expenseHeaderConstant),
...
//		$expenseHeaderText = ($expense->is_non_tax) ? '' : 'PT. Lanusa';
...
//            'expenseHeaderText' => $expenseHeaderText,
    ...
    public function actionAjaxJsonCodeNumberAccount($id) {
        if (Yii::app()->request->isAjaxRequest) {
            ...
            $accounts = Account::model()->findAllByAttributes(array('branch_id' => $expense->header->branch_id), 'account_category_id IN (1, 2)');

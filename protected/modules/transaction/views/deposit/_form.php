<div class="form">
    ...
    <?php echo CHtml::errorSummary($deposit->header); ?>
    ...
    <div class="container">
        <div class="span-12">
            ...
            <div class="row">
                <?php echo CHtml::activeLabelEx($deposit->header, 'branch_id'); ?>
                <?php if ($deposit->header->isNewRecord): ?>
                    <?php echo CHtml::activeDropDownList($deposit->header, 'branch_id', CHtml::listData(Branch::model()->findAll(), 'id', 'name'), array('empty' => '-- Pilih Perusahaan --',
                        'onchange' => CHtml::ajax(array(
                            'type' => 'POST',
                            'dataType' => "JSON",
                            'url' => CController::createUrl('ajaxJsonCodeNumberAccount', array('id' => $deposit->header->id)),
                            'success' => 'function(data) {
                                $("#code_number").html(data.codeNumber);
                                $("#' . CHtml::activeId($deposit->header, 'account_id') . '").html(data.accountOptions);
                            }',
                        )) . '$.fn.yiiGridView.update("account-grid", {
                            data: $("form").serialize()
                        });',
                    )); ?>
                    <?php echo CHtml::error($deposit->header, 'branch_id'); ?>
                <?php else: ?>
                    <?php echo CHtml::encode(CHtml::value($deposit->header, 'branch.name')); ?>
                <?php endif; ?>
            </div>

            <div class="row">
                <?php echo CHtml::activeLabelEx($deposit->header, 'account_id'); ?>
                <?php echo CHtml::activeDropDownList($deposit->header, 'account_id', CHtml::listData(Account::model()->findAllByAttributes(array('branch_id' => $deposit->header->branch_id), array('order' => 't.name', 'condition' => 'account_category_id IN (1, 2)')), 'id', 'name'), array('empty' => '-- Pilih Akun --')); ?>
